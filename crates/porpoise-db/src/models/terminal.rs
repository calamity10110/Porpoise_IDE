...
impl TerminalRow {
    ...
    pub fn find_by_id(pool: &DbPool, id: &str) -> Result<Option<Self>> {
        let conn = pool.get()?;
        let mut stmt = conn
            .prepare("SELECT id, session_id, rows, cols, shell, created_at FROM terminals WHERE id = ?1")
    ...
    pub fn list_by_session(pool: &DbPool, session_id: &str) -> Result<Vec<Self>> {
        let conn = pool.get()?;
        let mut stmt = conn
            .prepare("SELECT id, session_id, rows, cols, shell, created_at FROM terminals WHERE session_id = ?1")
            .map_err(|e| PorpoiseError::Db(format!("prepare failed: {e}")))?;
...
impl HistoryRow {
    ...
    pub fn query_by_terminal(pool: &DbPool, terminal_id: &str, limit: i64) -> Result<Vec<Self>> {
        ...
        let rows = stmt
            .query_map(rusqlite::params![terminal_id, limit], |row| {
                Ok(HistoryRow {
                    id: row.get(0)?,
