...
use crate::{
    frame::{Frame, FrameFlags, HEADER_SIZE, PROTOCOL_VERSION},
                            ...
                            match read_frame(&mut stream).await {
                                Ok(frame) => {
                                    ...
                                    if let Ok(WireMessage::Request(req)) = serde_json::from_slice(&frame.payload) {
                                        ...
                                        let payload =
                                            serde_json::to_vec(&WireMessage::Response(resp)).unwrap_or_default();
                                        let _ =
                                            write_frame(&mut stream, &Frame::new(FrameFlags::RESPONSE, payload)).await;
                                    }
                                }
                                Err(e) => {
