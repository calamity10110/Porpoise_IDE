...
use crate::types::{AgentKind, AgentOutput, OutputType};
...
impl Agent for ClaudeCodeAgent {
    ...
    async fn spawn(&self, worktree: &Path) -> Result<Box<dyn AgentHandle>> {
        if !crate::detector::AgentDetector::binary_in_path(&self.binary) {
            return Err(PorpoiseError::AgentNotDetected { kind: "claude".into() });
        }
        let child = tokio::process::Command::new(&self.binary)
            ...
            .spawn()
            .map_err(|e| PorpoiseError::Agent(format!("spawn claude: {e}")))?;
        let pid = child.id();
...
impl AgentHandle for ClaudeHandle {
    ...
    async fn read_output(&mut self) -> Result<Option<AgentOutput>> {
        if let Some(ref mut child) = self.child {
            ...
            if output.is_empty() { Ok(None) } else { Ok(Some(AgentOutput { text: output, output_type: OutputType::Stdout })) }
        } else { Ok(None) }
    }
    ...
    async fn shutdown(&mut self) -> Result<()> {
        if let Some(mut child) = self.child.take() {
            child.kill().await.map_err(|e| PorpoiseError::Agent(format!("kill: {e}")))?;
            child.wait().await.ok();
        }
        Ok(())
    }
